...
static void
daos_fio_queued(struct thread_data *td, struct io_u **io_us, int nr)
{
	int i;
	struct io_u *io_u;
	struct timespec now;
...
static int
daos_fio_commit(struct thread_data *td)
{
	struct daos_data *dd = td->io_ops_data;
	struct io_u *io_u;
	struct daos_iou *io;
	...
	for (i = 0; i < dd->queued; i++) {
		...
		if (io_u->ddir == DDIR_READ) {
			rc = dfs_read(dfs, dd->obj, &io->sgl, io_u->offset,
				      &io->read_size, &io->ev);
			if (rc) {
			...
			rc = dfs_write(dfs, dd->obj, &io->sgl, io_u->offset,
				       &io->ev);
			if (rc) {
